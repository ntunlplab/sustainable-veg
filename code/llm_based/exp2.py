...
def parse_args():
    ...
    parser.add_argument("--boxex", type=str, default="../ocr/data/ocr_output", help="path to OCR boxex files")
    parser.add_argument("--prompt", type=str, default="prompt2.txt", help="path to prompt template file")
...
def llm_predict(client, prompt, image_path):
    # print(prompt)
...
def process(data, args, client):
    ...
    for item in tqdm(data):
        ...
        for i, chunk in enumerate(ocr_data):
            ...
            response = llm_predict(client, prompt, filename)
            parsed = parse_response(response)

            # print for debugging
            print(f"Instance ID: {instance_id}, Chunk {i}, Label: {label}")
            print("========== Prompt ==========")
            print(prompt)
            print("======== Response =========")
            print(response)
...
def main():
    ...
    with open("test_predictions_4o-mini_v1.json", 'w') as f:
    ...
    with open("test_predictions_format_4o-mini_v1.json", 'w') as f:
        json.dump(test_predictions_format, f, indent=4)
    with open("test_raw_responses_4o-mini_v1.json", 'w') as f:
