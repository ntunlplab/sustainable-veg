...
def parse_args():
    ...
    parser.add_argument("--boxex", type=str, default="../ocr/ocr_olm/data/ocr_output", help="path to OCR boxex files")
    parser.add_argument("--prompt", type=str, default="prompt2-1.txt", help="path to prompt template file")
    parser.add_argument("--voting", type=int, default=5, help="number of votes for majority voting")
...
def apply_prompt_template(template_path, box_content, metrics_data):
    ...
    prompt = prompt.replace("{metrics_data}", sasb_stringify(metrics_data))
...
def process(data, args, client):
    ...
    for item in tqdm(data):
        ...
        for i, chunk in enumerate(ocr_data):
            ...
            bins = {}
            for v in range(args.voting):
                prompt = apply_prompt_template(args.prompt, box_content, metrics_data)
                response = llm_predict(client, prompt)
                parsed = parse_response(response)

                # print for debugging
                print(f"Instance ID: {instance_id}, Chunk {i}, Vote {v+1}")
                print("========== Prompt ==========")
                print(prompt)
                print("======== Response =========")
                print(response)
                ...
                print("\n\n")
                print("-------------------------------")
                print(f"Progress: {counter + 1}/{TOTAL_CHUNK_SIZE * args.voting}")

                if parsed is not None:
                    if parsed not in bins:
                        bins[parsed] = 0
                    bins[parsed] += 1
                else:
                    if "NONE" not in bins:
            ...
            # store voting result
            all_votings[f"chunk_{i}"] = bins

            print(f"Instance ID: {instance_id}, Chunk {i}, Voting result: {parsed} with {max_votes} votes")
        ...
        voting_results[instance_id] = all_votings


    return predictions, predictions_format, raw_responses, voting_results
...
def main():
    ...
    test_predictions, test_predictions_format, raw_responses, voting_results = process(test_data, args, client)
    ...
    with open("test_predictions_4o-mini_v2-3.json", 'w') as f:
    ...
    with open("test_predictions_format_4o-mini_v2-3.json", 'w') as f:
        json.dump(test_predictions_format, f, indent=4)
    with open("test_raw_responses_4o-mini_v2-3.json", 'w') as f:
        json.dump(raw_responses, f, indent=4)
    with open("test_voting_results_4o-mini_v2-3.json", 'w') as f:
