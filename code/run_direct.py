...
# from path_utils import CODE_DIR, PROJECT_ROOT, resolve_path
from path_utils import CODE_DIR, load_config

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--prompt", type=str, default=str("code/prompt/prompt_vlm.txt"), help="prompt template file")
...
def process(data, args, client, config):
    ...
    for item in tqdm(data):
        ...
        pdf_path = os.path.join(config["paths"]["esg_reports_pdf"], esg_report_name)
        metrics_data, _ = load_metrics(config['paths']['sasb_metrics'], item["sasb_report"].replace(".pdf", ""))
...
def main():
    ...
    pred_format, raw, stats = process(data, args, client, config)
